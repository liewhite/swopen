...
class Bip44Test extends AnyFunSuite {
  ...
  test("master account") {
    assert {
      ...
      account.address.bytes.toHex().toLowerCase == "0x48A0De73A57fDb5182b14C86592703f6A79B7993".toLowerCase &&
  ...
  test("account 1") {
    assert {
      ...
      account.address.bytes.toHex().toLowerCase == "0x116b68bA80C67909b28c77002c21A05e7D5C914B".toLowerCase &&
  ...
  test("account 2") {
    assert {
      ...
      account.address.bytes.toHex().toLowerCase == "0xb1973b0436d4A30ccCf3beF60D8aebc9B827E2F3".toLowerCase &&
