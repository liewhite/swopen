...
object ABIPack {
    inline given ABIPack[Unit] = new ABIPack[Unit] {
        ...
        def unpack(bytes: Array[Byte]): Unit = ()
    }
    ...
    inline given [T <: Tuple]: ABIPack[T] = new ABIPack[T] {
        ...
        def unpack(bytes: Array[Byte]): T = {
            ...
            val result   = packs.foldLeft(
              (List.empty[Any], staticOffset)
                ...
                if (!item.dynamic) {
                    ...
                    val dynamicOffset = bytes.slice(acc._2, acc._2 + 32).toBigUint
            ...
            Tuple.fromArray(result._1.toArray).asInstanceOf[T]
        }
    }
}
